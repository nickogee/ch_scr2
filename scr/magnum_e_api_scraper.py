from scr.magnum_e_fetchs import PARAMS_TOKEN, URL_TOKEN, URL_CATEGORY, PARAMS_CATEGORY, \
...
class Magnum_E_Scrapper():
    ...
    def fill_category_table(self):
        ...
        if not table_exists(db_path=DB_PATH, table_name=DB_MGM_E_CATEGORY_TABLE):
    ...
    #                                 self.category_list.append(dct)
            ...
            if self.category_list:
                upload_to_db(rezult=self.category_list, 
                            db_path=DB_PATH, 
                            table_name=DB_MGM_E_CATEGORY_TABLE,
                            table_create_str=DB_MGM_E_CATEGORY_CREATE_STR,
                            pk_column='id')
            ...
            for page in range(21):
                ...
                if content:
                    ...
                    for prod_dct in content:                      
                        ...
                        'category_full_path': f'{cat_tpl[2]}/{cat_tpl[1]}',
                        'brand': '',
    ...
    def __upload_to_db(self):
        ...
        print(f'Magnum express - получено {len(self.rezult)} sku')
        upload_to_db(self.rezult, DB_PATH, DB_ROW_DATA_TABLE, DB_ROW_DATA_CREATE_STR, 'product_id')
        
        # обновляем scrap_count для "спарсеных" категорий 2-го уровня
        update_category_mgm_air(self.category_update, DB_PATH, DB_MGM_E_CATEGORY_TABLE, 'id')
        ...
        parent_set = {i[3] for i in self.category_list}
        for par_id in parent_set:
            filter_tpl = ('parent_id', par_id)
            update_parent_category_mgm_air(db_path=DB_PATH, table_name=DB_MGM_E_CATEGORY_TABLE, pk_column='id', filter_tpl=filter_tpl)
...
def fast_category_scraper():
    ...
    fast_category = '3301, 3302, 3303, 3304'

    magnum_e = Magnum_E_Scrapper(fast_category_id=fast_category)
    magnum_e.start()


def main():
    magnum_e = Magnum_E_Scrapper()
    magnum_e.start()
